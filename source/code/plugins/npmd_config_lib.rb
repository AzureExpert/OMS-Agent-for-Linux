...
module NPMDConfig
    ...
    class AgentConfigCreator
        ...
        def self.getErrorSummary
            ...
            if @@agent_ip_drops != 0
                _agentIpDrops = "Agent IPs=#{@@agent_ip_drops}"
            end
            if @@agent_drops != 0
                _agentDrops= "Agents=#{@@agent_drops}"
            end
            if @@network_subnet_drops != 0
                _networkSNDrops = "Network subnets=#{@@network_subnet_drops}"
            end
            if @@network_drops != 0
                _networkDrops = "Networks=#{@@network_drops}"
            end
            if @@rule_subnetpair_drops != 0
                _ruleSNPairDrops = "Rule subnetpairs=#{@@rule_subnetpair_drops}"
            end
            if @@rule_drops != 0
                _ruleDrops = "Rules=#{@@rule_drops}"
            end
